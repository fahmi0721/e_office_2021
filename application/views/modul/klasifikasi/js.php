            ...
            if(parseInt(res['jumlah_data']) > 0){
                ...
                for(var i=0; i < res['data'].length; i++){
                    ...
                    html += "<tr>";
                        ...
                        html += "<td>";
                            html += iData['Nama']+"<br><small><b>Kode : "+iData['Kode']+"</b></small>";
                        html += "</td>";
                        ...
                        html += "<td class='text-center'>";
                            html += "<div class='btn-group'><a href='<?= base_url() ?>klasifikasi/edit/"+iData['Id']+"' class='btn btn-primary btn-xs' data-toggle='tooltip' title='ubah data'><i class='fa fa-edit'></i></a><a href='javascript:void(0)' onclick=\"HapusData('"+iData['Id']+"')\"  class='btn btn-danger btn-xs' data-toggle='tooltip' title='hapus data'><i class='fa fa-trash-o'></i></a></div>";
                        html += "</td>";
                    html += "</tr>";
                    No++;
                }
                ...
                $("#ShowData").html("<tr><td class='text-center' colspan='3'>no data availible in table</td></tr>");
                StopLoad();
            }
        },
        error : function(er){
...
function Validasi(){
    var iForm = ["Kode","Nama"];
    var iKet = ["Kode belum lengkap!","Nama belum lengkap"];
    for(i=0; i < iForm.length; i++){
        if($("#"+iForm[i]).val() == ""){
            Customerror("KLFKS", "001", iKet[i],"proses");
            scrolltop();
            $("#"+iForm[i]).focus();
            return false;
        }
...
function SubmitDataUpdate(){
    if(Validasi() != false){
        ...
        $.ajax({
            type : "POST",
            url : "<?= base_url('klasifikasi/update/') ?>",
            data : iData,
            success: function(res){
                var r = JSON.parse(res);
                if(r['status'] == "sukses"){
                    Customsukses("KLFKS", "001", r['pesan'], "proses");
                    setTimeout(function(){
                        $("#proses").html("");
                    },1000);
                    scrolltop();
                    ...
                    Customerror("KLFKS", "001", r['pesan'], "proses");
                    scrolltop();
                }
